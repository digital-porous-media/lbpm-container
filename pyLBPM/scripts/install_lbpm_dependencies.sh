...
export ZLIBSRC=zlib-1.3.1.tar.gz
if [ -f "$ZLIBSRC" ]; then
...
export LBPM_MPI_DIR=$DESTDIR/openmpi/$MPI_VERSION
export LBPM_HDF5_DIR=$DESTDIR/hdf5/$HDF5_VERSION
export PATH=$LBPM_MPI_DIR/bin:$PATH
export LD_LIBRARY_PATH=$LBPM_MPI_DIR/lib:$LBPM_HDF5_DIR/lib:$LD_LIBRARY_PATH
export LBPM_ZLIB_DIR=$DESTDIR/zlib/1.3.1
export LBPM_SZIP_DIR=$DESTDIR/szip/

mkdir -p $LBPM_MPI_DIR
...
if [[ -f "${NVCC}" ]]; then
    ...
    if [[ $GPU_NAME == "NVIDIA A100-PCIE-40GB" || $GPU_NAME == "NVIDIA RTX 6000 Ada Generation" ]]; then
...
if [[ ${OMPI_VERSION} == "Open MPI v$MPI_VERSION" ]]; then
    export LBPM_MPI_DIR=$(echo $OMPI_INFO | sed 's|/bin/ompi_info||g')
    export BUILD_MPI=false
    echo "Located existing Open MPI. Trying the version installed at $LBPM_MPI_DIR"
fi
...
if [[ ${BUILD_MPI} == true ]]; then
   echo "Install MPI to $LBPM_MPI_DIR"
   tar -xzvf $MPISRC
   ...
   if [[ -f "${CUDADIR}/bin/nvcc" ]]; then
       ...
       ./configure --with-cuda=${CUDADIR} --enable-mpi-thread-multiple --prefix=${LBPM_MPI_DIR}
   else
     echo "Building openmpi without GPU support"
     ./configure --enable-mpi-thread-multiple --prefix=$LBPM_MPI_DIR
   fi
...
if [[ ${BUILD_HDF5} == true ]]; then
    ...
    tar -xzvf $SZIPSRC
    cd szip-2.1.1
    CC=$LBPM_MPI_DIR/bin/mpicc CXX=$LBPM_MPI_DIR/bin/mpicxx CXXFLAGS="-fPIC -O3 -std=c++14" ./configure --prefix=$LBPM_SZIP_DIR && make && make install
    ...
    tar -xzvf $ZLIBSRC
    cd zlib-1.3
    CC=$LBPM_MPI_DIR/bin/mpicc CXX=$LBPM_MPI_DIR/bin/mpicxx CXXFLAGS="-fPIC -O3 -std=c++14" ./configure --prefix=$LBPM_ZLIB_DIR && make && make install
    cd $SRCDIR

   tar -xzvf $HDFSRC
   cd hdf5-1.14.3
   CC=$LBPM_MPI_DIR/bin/mpicc CXX=$LBPM_MPI_DIR/bin/mpicxx CXXFLAGS="-fPIC -O3 -std=c++14" ./configure --prefix=$LBPM_HDF5_DIR --enable-parallel --enable-shared --with-zlib=$LBPM_ZLIB_DIR
   make -j8 && make install
   cd $SRCDIR
   #--with-szip=$LBPM_SZIP_DIR
fi
...
echo "export MPICC=$LBPM_MPI_DIR/bin/mpicc"  >> $LBPM_CONFIG_DIR/config.sh
echo "export MPICXX=$LBPM_MPI_DIR/bin/mpicxx"  >> $LBPM_CONFIG_DIR/config.sh
echo "export LBPM_MPI_DIR=$LBPM_MPI_DIR"  >> $LBPM_CONFIG_DIR/config.sh
echo "export LBPM_HDF5_DIR=$LBPM_HDF5_DIR"  >> $LBPM_CONFIG_DIR/config.sh
echo "export LD_LIBRARY_PATH=$LBPM_MPI_DIR/lib:$LBPM_HDF5_DIR/lib:$LD_LIBRARY_PATH" >> $LBPM_CONFIG_DIR/config.sh
echo "export PATH=$LBPM_MPI_DIR/bin:$PATH" >> $LBPM_CONFIG_DIR/config.sh
